    ...
    @section('content-helper')
        <div class="col-sm-2 ml-auto">
            <a href="{{ url('blog/create') }}">
                <button type="button" class="btn btn-block btn-info btn-sm">Create Blog</button>
            </a>
        </div>
    @endsection
    @section('content')
                        ...
                        <tbody>
                            @foreach ($data as $item)
                                <tr>
                                    <td>{{ $item->title }}</td>
                                    <td>
                                        <button type="button" onclick="viewImage('{{ $item->image }}', '{{ $item->title }}')" class="btn btn-block btn-outline-secondary btn-xs" data-toggle="modal" data-target="#modal-image">
                                    ...
                                    <td>
                                        <div class="row">
                                            <div class="col-md-6">
                                                <a href="{{ url('/blog/update/'.$item->uuid) }}">
                                            ...
                                            <div class="col-md-6">
                                                <a href="#" data-toggle="modal" data-target="#modal-confirmation-delete">
                                                    <button type="button" class="btn btn-block btn-outline-secondary btn-xs" onclick="deleteItem('{{ url('/blog/delete/'.$item->uuid) }}')"><i class="fas fa-trash"></i>Delete</button>
                                                </a>
                                            </div>
                                        </div>
                                    </td>
